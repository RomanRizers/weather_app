...
Так как Open Meteo API, поддерживает только поиск по долготе и широте (может быть я чего-то не увидел), то нужно найти датасет, в котором будут находиться координаты городов мира.
...
В этом проекте мы использовали датасет с информацией о городах для наполнения базы данных. Вот шаги, которые мы предприняли для миграции данных:
